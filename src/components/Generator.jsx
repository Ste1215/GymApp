    ...
    return (
        <div className='flex flex-col gap-4'>
            <div className='flex items-center justify-center gap-2'>
                <p className='text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-semibold text-slate-400'>{index}</p>
                <h4 className=' ml-2 text-xl sm:text-2xl md:text-3xl'>{title}</h4>
            </div>
...
const Generator = () => {
  ...
  return (
    ...
    title={['It\'s','Huge','o\'clock']}>
        ...
        description={'Selectyour ultimate objective.'}/>
        <div className='grid grid-cols-3  gap-4'>
            {Object.keys(SCHEMES).map((schemes,schemesIndex) => {
                    return (
                        <button onClick={()=>{
                            setGoal(schemes)
                        }} className={ ' bg-slate-950 border border-blue-400 rounded-lg py-3 duration-200 hover:border-blue-600 ' + (schemes === goal ? 'border-blue-600' : 'border-blue-400')} key={schemesIndex}>
